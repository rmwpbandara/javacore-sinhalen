...
if (lock.tryLock(2, TimeUnit.SECONDS)) {   // 2s බලනවා, නැත්නම් අත්හරිනවා
  ...
  '2.4.6.1': {
    summary:
      'Executors framework = manual `new Thread()` වෙනුවට **thread pools** — threads reuse කරලා, bounded, managed. `newFixedThreadPool`, `newCachedThreadPool`, `newScheduledThreadPool`, හෝ full-control `ThreadPoolExecutor`.',
    sinhala: [
      {
        heading: 'කතාව: හැම task එකකටම new Thread — නරකයි',
        body: 'Mortar එකට sync tasks දහස් ගණන් තියෙනවා. හැම එකකටම `new Thread()` කරොත්? (1) Thread එකක් හදන එක expensive (OS resource). (2) Threads ගාන unbounded — දහසක් tasks = දහසක් threads = memory out (OOM/crash). ඕන වෙන්නේ — fixed threads pool එකක් තියාගෙන, tasks ඒවා අතරේ reuse කරන එක. ඒකට **ExecutorService** (thread pool).',
      },
      {
        heading: 'Pool වර්ග',
        body: 'Executors factory methods:',
        points: [
          '`newFixedThreadPool(n)` — fixed n threads. Tasks queue වෙනවා. Bounded, predictable — most common.',
          '`newCachedThreadPool()` — demand අනුව threads grow/shrink (idle threads reclaim). Bursty short tasks වලට.',
          '`newScheduledThreadPool(n)` — delayed / periodic tasks (`scheduleAtFixedRate`). Cron වගේ.',
          '`newVirtualThreadPerTaskExecutor()` (Java 21) — virtual threads (3.2.7), massive IO concurrency.',
          '`ThreadPoolExecutor` — core/max size, queue, rejection policy — full control.',
        ],
      },
      {
        heading: 'හැමවිටම shutdown',
        body: 'Pool එකක් වැඩ ඉවර වුනාම `shutdown()` කරන්නම ඕන — නැත්නම් threads දිගටම run වෙලා program එක exit වෙන්නෙ නෑ (resource leak). `shutdown()` = අලුත් tasks accept නෑ, existing ඉවර වෙනවා. `awaitTermination()` වලින් ඉවර වෙනකම් බලනවා. try-with-resources (Java 19+) එකෙනුත් auto-close.',
      },
    ],
    analogy:
      'Call center එකක් වගේ — හැම call එකකටම අලුත් agent කෙනෙක් බඳවා ගන්නෙ නෑ (expensive + unbounded). Fixed agents pool එකක් (thread pool) calls (tasks) queue එකෙන් අරන් handle කරනවා. Center එක වහනකොට agents ලා නිවාඩු දෙනවා (shutdown).',
    code: [
      {
        filename: 'FixedPool.java',
        language: 'java',
        code: `// connectors 15ක් — pool එකේ 8 threads reuse කරලා parallel sync
ExecutorService pool = Executors.newFixedThreadPool(8);

for (Connector c : connectors) {
    pool.submit(c::sync);       // task queue එකට — 8 threads reuse
}

pool.shutdown();                // අලුත් tasks නෑ; existing ඉවර වෙනවා
pool.awaitTermination(5, TimeUnit.MINUTES);   // ඉවර වෙනකම් බලනවා`,
        note: 'Fixed pool = bounded threads reused; shutdown() අමතක කරන්න එපා.',
      },
      {
        filename: 'ScheduledPool.java',
        language: 'java',
        code: `// periodic task — හැම පැයකම churn scores recompute
ScheduledExecutorService sched = Executors.newScheduledThreadPool(1);

sched.scheduleAtFixedRate(
    () -> recomputeChurn(),
    0,                  // initial delay
    1, TimeUnit.HOURS   // period
);`,
        note: 'ScheduledExecutorService = delayed/periodic tasks (cron වගේ).',
      },
      {
        filename: 'TryWithResourcesPool.java',
        language: 'java',
        code: `// Java 19+ — ExecutorService AutoCloseable, auto shutdown
try (ExecutorService pool = Executors.newFixedThreadPool(8)) {
    for (Connector c : connectors) pool.submit(c::sync);
}   // block ඉවර වෙනකොට auto shutdown + await (2.1.5)`,
        note: 'Modern: try-with-resources → auto shutdown.',
      },
    ],
    mortar:
      'Mortar connectors 15+ ක් parallel sync කරන්න `newFixedThreadPool` — bounded threads reuse, OOM නෑ. Nightly churn/segment recompute `ScheduledExecutorService`. IO-heavy enrichment (millions of API calls) → Java 21 virtual thread executor (3.2.7). Manual `new Thread` කවදාවත් නෑ — pools = stable, bounded, tunable, observable (queue depth, active threads metrics).',
    keyPoints: [
      'Thread pools = threads reuse + bounded resources (OOM වළක්වයි).',
      'Fixed / Cached / Scheduled / Virtual / custom ThreadPoolExecutor.',
      'හැමවිටම `shutdown()` (හෝ try-with-resources) — resource leak වළක්වන්න.',
      'Manual `new Thread` avoid — pools use කරන්න.',
    ],
    pitfalls: [
      '`shutdown()` නොකර pool එකක් අත්හැරියොත් — non-daemon threads run වෙලා JVM exit වෙන්නෙ නෑ.',
      'Unbounded `newCachedThreadPool` + slow tasks = threads පිපිරෙනවා — bounded fixed pool safer.',
    ],
  },

  '2.4.6.2': {
    summary:
      '`Future` = async result placeholder (`.get()` blocks). `CompletableFuture` = composable async pipelines — `thenApply` (transform), `thenCompose` (chain), `thenCombine` (join), `exceptionally` (error), non-blocking.',
    sinhala: [
      {
        heading: 'කතාව: Future.get() එකේ ප්‍රශ්නෙ',
        body: 'Callable + Future (2.4.2) එකෙන් async result ගන්න පුළුවන් — ඒත් `future.get()` **blocks** (result එනකම් thread එක නවතිනවා). Results කිහිපයක් chain කරන්න ඕන නම් (fetch → transform → save), Future වලින් ugly nested blocking code. `CompletableFuture` මේක fix කරනවා — non-blocking, composable pipelines. "result ready වුනාම මේක කරන්න, ඊට පස්සේ මේක" කියලා declaratively chain කරනවා.',
      },
      {
        heading: 'CompletableFuture operations',
        body: 'ප්‍රධාන methods:',
        points: [
          '`supplyAsync(() -> ...)` — async task එකක් පටන් ගන්නවා (result එකක් සමඟ).',
          '`thenApply(fn)` — result එක transform කරනවා (sync).',
          '`thenCompose(fn)` — තව CompletableFuture එකක් chain කරනවා (flat, nested නෑ).',
          '`thenCombine(other, fn)` — independent futures දෙකක් join කරනවා.',
          '`exceptionally(fn)` / `handle(fn)` — errors gracefully handle කරනවා.',
        ],
      },
    ],
    analogy:
      '`Future.get()` = queue ticket එකක් — ආපහු ඇවිත් "ready ද?" අහන්න ඕන (block වෙනවා). `CompletableFuture` = "ready වුනාම මට SMS එකක් දාන්න, ඊට පස්සේ මේකත් කරන්න, අවුලක් වුනොත් මේ විදිහට handle කරන්න" කියන smart, non-blocking pipeline එකක්.',
    code: [
      {
        filename: 'EnrichmentPipeline.java',
        language: 'java',
        code: `// enrichment stages chain — non-blocking, composable
CompletableFuture
    .supplyAsync(() -> loadCustomer(id))     // async fetch
    .thenApply(this::inferName)              // transform
    .thenApply(this::geocodeAddress)         // transform
    .thenApply(this::validateEmail)          // transform
    .thenAccept(repo::save)                  // consume (side effect)
    .exceptionally(ex -> {                   // error handling
        log.error("enrich fail", ex);
        return null;
    });`,
        note: 'Stages chain — blocking නැතුව, errors gracefully.',
      },
      {
        filename: 'CombineFutures.java',
        language: 'java',
        code: `// independent calls parallel — join කරනවා (8.4.3 API composition වගේ)
CompletableFuture<Profile> profile = CompletableFuture.supplyAsync(() -> loadProfile(id));
CompletableFuture<Churn>   churn   = CompletableFuture.supplyAsync(() -> loadChurn(id));

CompletableFuture<Customer360> combined = profile.thenCombine(churn,
    (p, c) -> new Customer360(p, c));       // දෙකම ready වුනාම join

Customer360 result = combined.join();       // parallel -> latency = slowest එක`,
        note: 'thenCombine = independent futures parallel + join.',
      },
    ],
    mortar:
      'Mortar enrichment pipeline (name inference → gender → geocode → email-validate) `CompletableFuture` chain එකක් — stages async, non-blocking, errors gracefully handled. Millions of profiles throughput වැඩියෙන් enrich කරනවා. Customer-360 aggregation (8.4.3) `thenCombine` වලින් resolution + prediction + activation parallel calls join කරනවා — latency = slowest service එක, sum නෙවෙයි. Blocking `Future.get()` chains වෙනුවට composable pipelines = better throughput + clean code.',
    keyPoints: [
      'Future = blocking `.get()`; CompletableFuture = non-blocking + composable.',
      'thenApply (transform) / thenCompose (chain) / thenCombine (join) / exceptionally (error).',
      'Async pipelines = better throughput, no thread blocking.',
      'Independent futures parallel → latency = slowest, not sum.',
    ],
    pitfalls: [
      '`thenApply` vs `thenCompose`: chain එකක් returns CompletableFuture නම් `thenCompose` (නැත්නම් nested `CompletableFuture<CompletableFuture<X>>`).',
      'CompletableFuture default common ForkJoinPool එකේ run වෙනවා — blocking IO වලට custom executor pass කරන්න (`supplyAsync(task, executor)`).',
    ],
  },

  '2.4.6.3': {
    summary:
      'Concurrent collections = purpose-built thread-safe collections. `ConcurrentHashMap` (scalable map), `BlockingQueue` (producer-consumer + back-pressure). Manual synchronization / `Collections.synchronizedX` වලට වඩා fast.',
    sinhala: [
      {
        heading: 'කතාව: synchronizedMap slow',
        body: 'Multi-threaded Mortar counters වලට thread-safe Map එකක් ඕන. `Collections.synchronizedMap()` use කරන්න පුළුවන් — ඒත් ඒක **whole map එකම lock** කරනවා හැම operation එකකටම (එකවර එක thread). Threads ගොඩක් නම් bottleneck. `ConcurrentHashMap` smart — map එක buckets වලට බෙදලා, bucket-level locking (වෙන buckets එකවර access කරන්න පුළුවන්). Scalable.',
      },
      {
        heading: 'ප්‍රධාන concurrent collections',
        body: 'නිතර use වෙන ඒවා:',
        points: [
          '`ConcurrentHashMap` — thread-safe map, fine-grained locking (scalable). `merge`/`compute` atomic.',
          '`BlockingQueue` (ArrayBlockingQueue, LinkedBlockingQueue) — producer-consumer. `put` full නම් block, `take` empty නම් block (wait/notify built-in).',
          '`CopyOnWriteArrayList` (2.2.2) — read-heavy concurrent list.',
          '`ConcurrentLinkedQueue` — lock-free (non-blocking) queue.',
        ],
      },
      {
        heading: 'BlockingQueue = back-pressure',
        body: 'BlockingQueue එකේ bounded capacity එකක් දුන්නම — producer consumer ට වඩා වේගවත් වුනොත්, queue එක full වෙලා producer **automatic block** වෙනවා (back-pressure). ඒ නිසා system එක overwhelm වෙන්නෙ නෑ — naturally throttle වෙනවා. Manual wait/notify (2.4.4) ඕන නෑ.',
      },
    ],
    analogy:
      '`synchronizedMap` = එක දොරක් තියෙන shop එකක් (එකවර එකෙක්). `ConcurrentHashMap` = departments කිහිපයක් තියෙන supermarket එකක් (වෙන departments එකවර). `BlockingQueue` = conveyor belt එකක් — belt full නම් producer නවතිනවා, empty නම් consumer බලාගෙන (automatic).',
    code: [
      {
        filename: 'ConcurrentMap.java',
        language: 'java',
        code: `// multiple sync threads — scalable thread-safe counters
Map<String, Long> counts = new ConcurrentHashMap<>();

// atomic — race condition නෑ, whole-map lock නෑ
counts.merge("shopify", 1L, Long::sum);
counts.compute("klaviyo", (k, v) -> (v == null ? 0 : v) + 1);`,
        note: 'ConcurrentHashMap = fine-grained locking + atomic merge/compute.',
      },
      {
        filename: 'BlockingQueueBackPressure.java',
        language: 'java',
        code: `// bounded queue — back-pressure built in
BlockingQueue<Batch> queue = new LinkedBlockingQueue<>(1000);   // capacity 1000

// producer (connector)
new Thread(() -> {
    try { queue.put(nextBatch()); }        // full නම් block (throttle)
    catch (InterruptedException e) { }
}).start();

// consumer (resolver)
new Thread(() -> {
    try { while (true) process(queue.take()); }  // empty නම් block
    catch (InterruptedException e) { }
}).start();`,
        note: 'BlockingQueue = producer-consumer + automatic back-pressure.',
      },
    ],
    mortar:
      'Mortar real-time streaming ingestion pipeline එකේ `BlockingQueue` back-pressure දෙනවා — producers (connectors) consumers (resolvers) ට වඩා වේගවත් වුනොත්, queue එක naturally throttle කරලා system එක overwhelm වෙන්නෙ නෑ. Shared sync counters, dedup indexes `ConcurrentHashMap` (scalable, atomic). Manual `synchronized` + wait/notify වෙනුවට මේ purpose-built collections = fast, correct, less bug-prone.',
    keyPoints: [
      'ConcurrentHashMap = scalable thread-safe map (fine-grained locking, atomic ops).',
      'BlockingQueue = producer-consumer + back-pressure (bounded capacity).',
      'Manual synchronized / synchronizedMap වලට වඩා fast + scalable.',
      'wait/notify (2.4.4) වෙනුවට BlockingQueue prefer.',
    ],
    pitfalls: [
      '`ConcurrentHashMap` null key/value allow කරන්නෙ නෑ (`NullPointerException`).',
      'Unbounded BlockingQueue = back-pressure නෑ (memory පිරෙනවා) — bounded capacity දෙන්න.',
    ],
  },

  '2.4.6.4': {
    summary:
      '`ReentrantLock` = `synchronized` වලට වඩා flexible lock (`tryLock` timeout, fairness, interruptible). `ReadWriteLock` = concurrent reads (shared) + exclusive writes — read-heavy data වලට throughput වැඩි.',
    sinhala: [
      {
        heading: 'කතාව: synchronized මදි වෙන තැන',
        body: '`synchronized` (2.4.3) simple, ඒත් limited — lock එක ගන්න forever බලාගෙන ඉන්නවා (timeout නෑ), fairness නෑ, interrupt කරන්න බෑ. Mortar cache එකක් read-heavy (millions of lookups, rare updates) නම් — `synchronized` හැම read එකකටම exclusive lock (reads එකවර වෙන්නෙ නෑ!). ඕන වෙන්නේ — reads ගොඩක් එකවර, writes විතරක් exclusive. ඒකට advanced locks.',
      },
      {
        heading: 'ReentrantLock + ReadWriteLock',
        body: 'java.util.concurrent.locks:',
        points: [
          '`ReentrantLock` — synchronized වගේ, ඒත් extra: `tryLock(timeout)` (block වෙන්නෙ නැතුව), `lockInterruptibly()`, fairness policy.',
          '`ReadWriteLock` — locks දෙකක්: read lock (**shared** — reads කිහිපයක් එකවර) + write lock (**exclusive** — එකෙක් විතරයි).',
          'Read-heavy data (rare writes) → ReadWriteLock reads parallel කරලා throughput වැඩි.',
          'රීතිය: `lock()` කරාම හැමවිටම `finally { unlock(); }` — නැත්නම් permanent lock!',
        ],
      },
    ],
    analogy:
      '`ReadWriteLock` = library reading room එකක් වගේ. පොත කියවන්න (read) ගොඩක් අයට එකවර පුළුවන් (shared). ඒත් පොත edit කරනකොට (write) කවුරුවත් ඇතුලට දෙන්නෙ නෑ, එකෙක් විතරයි (exclusive). Reading = concurrent, editing = exclusive.',
    code: [
      {
        filename: 'ReadWriteLock.java',
        language: 'java',
        code: `class ConfigCache {
    private final Map<String, String> cache = new HashMap<>();
    private final ReadWriteLock rw = new ReentrantReadWriteLock();

    // READ — shared (reads කිහිපයක් එකවර)
    String get(String key) {
        rw.readLock().lock();
        try { return cache.get(key); }
        finally { rw.readLock().unlock(); }    // හැමවිටම finally
    }

    // WRITE — exclusive (එකෙක් විතරයි)
    void put(String key, String val) {
        ...
        note: 'Reads shared (parallel), writes exclusive → read-heavy throughput වැඩි.',
      },
      {
        filename: 'ReentrantTryLock.java',
        language: 'java',
        code: `ReentrantLock lock = new ReentrantLock();

// tryLock — forever බලාගෙන ඉන්නෙ නැතුව
if (lock.tryLock(2, TimeUnit.SECONDS)) {
    try { /* critical work */ }
    finally { lock.unlock(); }              // finally!
} else {
    log.warn("lock timeout — deadlock වළක්වනවා");   // graceful (2.4.5)
}`,
        note: 'ReentrantLock = tryLock/timeout/fairness (synchronized+).',
      },
    ],
    mortar:
      'Mortar in-memory config / recommendation caches read-heavy (ගොඩක් lookups, rare updates). `ReadWriteLock` වලින් concurrent reads throughput දරාමට වැඩි කරලා, updates safe තියාගන්නවා. Cross-resource jobs `ReentrantLock` + `tryLock` timeout වලින් deadlock (2.4.5) වළක්වනවා. Simple counters වලට `Atomic*` (lock-free) — locks වඩාත් complex coordination වලට විතරයි.',
    keyPoints: [
      'ReentrantLock = tryLock(timeout) / interruptible / fairness (synchronized+).',
      'ReadWriteLock = concurrent reads (shared) + exclusive writes.',
      'Read-heavy data → ReadWriteLock (reads parallel).',
      '`lock()` → හැමවිටම `finally { unlock(); }`.',
    ],
    pitfalls: [
      '`unlock()` `finally` එකේ නොදැම්මොත් — exception එකකදී lock permanent held → deadlock.',
      'Read lock එකෙන් write lock එකට "upgrade" කරන්න බෑ (deadlock) — write lock එකෙන් read lock එකට "downgrade" පුළුවන්.',
    ],
  },

  '2.4.6.5': {
    summary:
      'Synchronizers = threads coordinate කරන high-level tools. `CountDownLatch` (N tasks ඉවර වෙනකම් බලනවා), `CyclicBarrier` (N threads එකතැනකට එනකම්), `Semaphore` (concurrent access limit — rate limiting).',
    sinhala: [
      {
        heading: 'කතාව: හැම sync එකම ඉවර වුනාට පස්සේ resolution',
        body: 'Mortar pipeline එකේ — connectors ඔක්කොම sync ඉවර වුනාට **පස්සේ** විතරයි identity resolution පටන් ගන්න ඕන. Threads 15ක් sync කරනවා; ඔක්කොම ඉවර වෙනකම් main thread එක බලාගෙන ඉන්නේ කොහොමද? Manual counters + wait/notify ugly. ඒකට ready-made tool එකක් — `CountDownLatch`. මේ වගේ coordination tools 3ක් තියෙනවා.',
      },
      {
        heading: 'තුනේ භාවිතය',
        body: 'Synchronizers 3:',
        points: [
          '`CountDownLatch(n)` — n tasks ඉවර වෙනකම් (`countDown()` n වතාවක්) එක thread එකක් `await()` කරලා බලනවා. One-time (reset නෑ).',
          '`CyclicBarrier(n)` — threads n ක්ම එකතැනකට (`await()`) එනකම් හැමෝම බලනවා, ඊට පස්සේ එකට යනවා. Reusable (cyclic).',
          '`Semaphore(permits)` — එකවර access කරන threads ගාන limit කරනවා (permits acquire/release). Rate limiting / resource pooling.',
        ],
      },
    ],
    analogy:
      '`CountDownLatch` = "හැම කෙනෙක්ම bus එකට ආවම පිටත් වෙනවා" (ඔක්කොම එනකම් බලනවා, එක පාරයි). `CyclicBarrier` = tour group එකක් හැම stop එකකදීම හැමෝම එකතු වෙනකම් බලනවා (නැවත නැවත). `Semaphore` = parking lot එකක permits — full නම් බලාගෙන, එකක් යනකම්.',
    code: [
      {
        filename: 'CountDownLatch.java',
        language: 'java',
        code: `// connectors ඔක්කොම sync ඉවර වෙනකම් බලලා -> resolution
CountDownLatch latch = new CountDownLatch(connectors.size());

for (Connector c : connectors) {
    pool.submit(() -> {
        try { c.sync(); }
        finally { latch.countDown(); }     // ඉවර වුනාම count අඩු කරනවා
    });
}

latch.await();                             // ඔක්කොම (count 0) වෙනකම් block
System.out.println("ඔක්කොම sync ඉවර -> resolution පටන් ගමු");
runResolution();`,
        note: 'CountDownLatch = "ඔක්කොම ඉවර වෙනකම්" barrier (one-time).',
      },
      {
        filename: 'Semaphore.java',
        language: 'java',
        code: `// external ad-platform API — එකවර max 5 calls (rate limit)
Semaphore api = new Semaphore(5);         // 5 permits

void callMetaApi() throws InterruptedException {
    api.acquire();                        // permit ගන්නවා (full නම් block)
    try {
        metaClient.pushAudience();        // max 5 concurrent
    } finally {
        api.release();                    // permit ආපහු දෙනවා
    }
}`,
        note: 'Semaphore = bounded concurrency / rate limiting.',
      },
    ],
    mortar:
      'Mortar "connect → cleanse → predict" pipeline එකේ, connectors ඔක්කොම sync ඉවර වුනාට පස්සේ විතරයි resolution පටන් ගන්නේ — `CountDownLatch`. External ad-platform APIs (Meta/Google) rate-limit කරන්න `Semaphore(5)` — max 5 concurrent calls (vendor quota respect කරනවා, 11.1.3). Multi-phase batch jobs threads sync කරන්න `CyclicBarrier`. මේ high-level synchronizers manual wait/notify වලට වඩා clear + bug-free.',
    keyPoints: [
      'CountDownLatch = N tasks ඉවර වෙනකම් බලනවා (one-shot).',
      'CyclicBarrier = N threads rendezvous (reusable).',
      'Semaphore = bounded concurrency / rate limiting (permits).',
      'Manual wait/notify වලට වඩා clear, purpose-built.',
    ],
    pitfalls: [
      'CountDownLatch reset කරන්න බෑ (one-time) — නැවත ඕන නම් CyclicBarrier.',
      'Semaphore `release()` `finally` එකේ නොදැම්මොත් — permits leak වෙලා eventually ඔක්කොම block.',
    ],
  },

  '2.4.7': {
    summary:
      '`ThreadLocal` = හැම thread එකකටම variable එකකේ **තමන්ගේම copy** එකක් — shared state නැතුව per-thread context. Request/tenant/trace context carry කරන්න common. Thread pools වල හැමවිටම `remove()`.',
    sinhala: [
      {
        heading: 'කතාව: current brandId එක හැම layer එකටම pass කරන්නද?',
        body: 'Mortar multi-tenant — හැම request එකකම brandId එකක් තියෙනවා. Controller → service → repository හැම method එකකටම `brandId` parameter එකක් pass කරන්නද? Ugly, error-prone. ඕන වෙන්නේ — current thread එකට "attach" කරපු, ඕන තැනකින් access කරන්න පුළුවන් context එකක්. ඒත් threads අතර share වෙන්නෙ නැතුව (thread A ගේ brandId thread B ට පේන්න බෑ). ඒකට `ThreadLocal`.',
      },
      {
        heading: 'ThreadLocal වැඩ කරන විදිහ',
        body: 'Per-thread storage:',
        points: [
          'හැම thread එකක්ම එකම `ThreadLocal` variable එකෙන් **තමන්ගේම independent value** එකක් දකිනවා.',
          'Shared mutable state නෑ → synchronization ඕන නෑ (thread-safe by design).',
          'Request context (userId, brandId, traceId) carry කරන්න ideal — parameters pass නොකර.',
          'රීතිය: thread pools (2.4.6.1) වල threads reuse වෙන නිසා, `finally { threadLocal.remove(); }` — නැත්නම් stale/leaked context අනිත් request එකට.',
        ],
      },
    ],
    analogy:
      'හැම employee කෙනෙක්ටම තමන්ගේම locker එකක් වගේ — එකම "locker" concept එක, ඒත් හැමෝටම වෙන වෙනම content. John ගේ locker එකේ දේවල් Mary ට පේන්නෙ නෑ. Office එකෙන් යනකොට locker එක හිස් කරන්නම ඕන (remove) — නැත්නම් ඊළඟ කෙනාට ඔයාගේ දේවල්.',
    code: [
      {
        filename: 'BrandContext.java',
        language: 'java',
        code: `public class BrandContext {
    // හැම thread එකකටම තමන්ගේම brandId copy එකක්
    private static final ThreadLocal<String> CURRENT = new ThreadLocal<>();

    public static void set(String brandId) { CURRENT.set(brandId); }
    public static String get() { return CURRENT.get(); }
    public static void clear() { CURRENT.remove(); }   // leak වළක්වන්න!
}`,
        note: 'ThreadLocal = per-thread independent value (no sharing, no locks).',
      },
      {
        filename: 'UsingContext.java',
        language: 'java',
        code: `// request handler — context set කරලා, finally එකේ clear
void handleRequest(String brandId) {
    BrandContext.set(brandId);
    try {
        runSync();      // පහළ layers වලට brandId parameter pass කරන්න ඕන නෑ
    } finally {
        BrandContext.clear();   // thread pool reuse -> stale context වළක්වන්න
    }
}

// deep in the call stack — parameter නැතුව access
void repositoryQuery() {
    String brand = BrandContext.get();   // current thread ගේ brandId
}`,
        note: 'Context ඕන තැනකින් access; thread pools වල finally එකේ clear() critical.',
      },
    ],
    mortar:
      'Mortar multi-brand platform එකේ, current request එකේ brandId/tenant/traceId `ThreadLocal` context එකක තියාගෙන, පහළ layers (service, repository, query builder) වලට parameter pass නොකර access කරනවා — tenant isolation (PROJECT_IDEA 10.5) + distributed tracing (8.2.6) වලට. Spring Security එකේ `SecurityContextHolder`, Sleuth/Micrometer trace context මේ pattern එකෙන්ම වැඩ කරනවා. Pool threads reuse වෙන නිසා `remove()` critical — නැත්නම් brand A ගේ data brand B request එකකට leak (security bug!).',
    keyPoints: [
      'ThreadLocal = per-thread independent copy (no sharing → thread-safe, no locks).',
      'Request/tenant/trace context carry කරන්න ideal (parameters නැතුව).',
      'Thread pools: `finally { remove(); }` — stale/leaked context වළක්වන්න.',
      'Spring SecurityContext / trace context මේ pattern එකෙන්ම.',
    ],
    pitfalls: [
      'Pool threads reuse නිසා `remove()` නොකළොත් — පරණ request එකේ context (brandId) ඊළඟ request එකට leak (tenant isolation bug + memory leak).',
      'ThreadLocal child threads / async (CompletableFuture) වලට automatic propagate වෙන්නෙ නෑ — manually pass කරන්න (`InheritableThreadLocal` හෝ context propagation).',
    ],
  },
};
